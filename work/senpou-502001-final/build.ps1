...
$release=Join-Path (Get-Location) 'releases\Nicky-Allowed-Invisible-alpha-0.1.5'
$out=Join-Path (Get-Location) 'dist\senpou-502001-final\mods\map\m20_00_00_00'
New-Item -ItemType Directory -Path $out -Force | Out-Null
$candidates=@('m20_00_00_00_502001')
$report=foreach($id in $candidates){
 ...
 foreach($entry in $b.Files | Where-Object Name -Match '\.flver$'){
  ...
  for($i=0;$i -lt $edited.Meshes.Count;$i++){
   ...
   if(@($mat.Textures | Where-Object Path -Match 'm20_worm_02_[an]\.tif$').Count){
    $targets+= $i
...
$report | ConvertTo-Json -Depth 10 | Set-Content work\senpou-502001-final\validation.json

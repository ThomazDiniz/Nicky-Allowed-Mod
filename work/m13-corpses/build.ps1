...
$release=Join-Path (Get-Location) 'releases\Nicky-Allowed-Invisible-alpha-0.1.4'
$out=Join-Path (Get-Location) 'dist\m13-corpses-hotfix\mods\map\m13_00_00_00'
New-Item -ItemType Directory -Path $out -Force | Out-Null
$candidates=@('m13_00_00_00_001200')
$report=foreach($id in $candidates){
 if($id -notmatch '^m13_00_00_00_'){throw 'Unexpected map'}
 $file=$id+'.mapbnd.dcx'
 $source=Join-Path $release "mods\map\m13_00_00_00\$file"
 if(!(Test-Path $source)){$source="$game\map\m13_00_00_00\$file"}
 ...
 foreach($entry in $b.Files | Where-Object Name -Match '\.flver$'){
  ...
  for($i=0;$i -lt $edited.Meshes.Count;$i++){
   ...
   if(@($mat.Textures | Where-Object Path -Match 'm13_worm_(01|body_01)_[an]\.tif$').Count){
    $targets+= $i
...
$report | ConvertTo-Json -Depth 10 | Set-Content work\m13-corpses\validation.json

...
$release=Join-Path $workspace 'releases\Nicky-Allowed-Invisible-alpha-0.1.4'
...
$backup=Join-Path $workspace ('work\backups\before-alpha-014-install-'+(Get-Date -Format 'yyyyMMdd-HHmmss'))
...
[pscustomobject]@{Version='0.1.4';Installed=$true;Packages=27;AllHashesMatch=$true;Backup=$backup;GameClosed=$true;GameTested=$false;Date=(Get-Date -Format o)} | ConvertTo-Json | Set-Content (Join-Path $workspace 'work\alpha-014-installation.json')
Get-Content (Join-Path $workspace 'work\alpha-014-installation.json')

...
$release=Join-Path $workspace 'releases\Nicky-Allowed-Invisible-alpha-0.1.5'
...
$backup=Join-Path $workspace ('work\backups\before-alpha-015-install-'+(Get-Date -Format 'yyyyMMdd-HHmmss'))
...
[pscustomobject]@{Version='0.1.5';Installed=$true;Packages=28;AllHashesMatch=$true;Backup=$backup;GameClosed=$true;GameTested=$false;Date=(Get-Date -Format o)} | ConvertTo-Json | Set-Content (Join-Path $workspace 'work\alpha-015-installation.json')
Get-Content (Join-Path $workspace 'work\alpha-015-installation.json')

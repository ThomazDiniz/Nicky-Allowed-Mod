...
$release=Join-Path $workspace 'releases\Nicky-Allowed-Invisible-Centipedes-alpha-0.1.6'
...
if($manifest.Count -ne 28){throw 'Unexpected manifest count'}
...
$backup=Join-Path $workspace ('work\backups\before-alpha-016-install-'+(Get-Date -Format 'yyyyMMdd-HHmmss'))
...
[pscustomobject]@{Version='0.1.6';Installed=$true;Packages=28;AllHashesMatch=$true;Backup=$backup;GameClosed=$true;GameTested=$false;Date=(Get-Date -Format o)} | ConvertTo-Json | Set-Content (Join-Path $workspace 'work\alpha-016-installation.json')
Get-Content (Join-Path $workspace 'work\alpha-016-installation.json')

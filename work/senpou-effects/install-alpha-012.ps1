...
if($manifest.Count -ne 26){throw 'Unexpected manifest count'}
...
$backup=Join-Path $workspace ('work\backups\before-alpha-012-install-'+(Get-Date -Format 'yyyyMMdd-HHmmss'))
...
[pscustomobject]@{Version='0.1.2';Installed=$true;Packages=26;AllHashesMatch=$true;Backup=$backup;GameClosed=$true;GameTested=$false;Date=(Get-Date -Format o)} | ConvertTo-Json | Set-Content (Join-Path $workspace 'work\alpha-012-installation.json')
Get-Content (Join-Path $workspace 'work\alpha-012-installation.json')

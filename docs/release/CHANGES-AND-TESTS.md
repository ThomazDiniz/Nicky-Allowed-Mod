...
The central project `VERSION` file controls the menu label, archive name, packaged VERSION and generated release documentation. All 28 earlier mod files are unchanged from alpha 0.1.6. The menu label indicates that the menu override loaded, not that every other mod file was independently verified by the game.
...
Only fixed-size visual field blocks were changed directly in the original FXR bytes. File size, structure, internal offsets and all bytes outside those blocks were preserved. Existing timing, movement and other properties remain, so this is a visual substitution rather than a copy of an entire blood attack. Other entries in the effect packages remained byte-for-byte identical to their originals.

The earlier 0.1.1 attempt removed effect components and was followed by a reported monk-grab crash. That approach was reverted. The current substitution, introduced in 0.1.4, was confirmed by the user to work without a crash in the tested encounter; the exact cause of the earlier crash was not proven.
...
- [x] Particle substitution confirmed by the user without a crash in the tested encounter.
...
The reference audit rechecked metadata for 14,424 models and read 2,913 original FXR files, finding no additional confirmed targets under the known references. This does not exclude other names, baked textures, shadows or unexamined visual appearances.
